...
# ]
# ///
"""Interactive fictional streaming-catalog learning lab."""
...
def _():
    ...
    Explore duration by release period and genre at one-fictional-title grain. Fixture identity displays `generator_version`, seed, row count, grain, and canonical CSV SHA-256.
    ...
    **Limitations:** Titles and catalog observations are synthetic. Descriptive duration patterns do not represent any real streaming service, audience, licensing catalog, or commercial outcome.
    """)
    return
    ...
    _table = (
        mo.Html(
            render_captioned_table(
                ...
                caption=f"Catalog duration evidence for seed {lab_run.seed}",
...
def _(lab_run):
    _chart = (
        mo.Html(
            render_bar_evidence(
                lab_run.result.primary_table,
                label_column="catalog_slice",
                value_column="mean_duration_minutes",
                title="Mean fictional title duration by catalog slice",
                description=(
                    "Horizontal bars show mean duration for the first release-period "
                    "and genre slices in the deterministic summary."
                ),
            )
        )
        if lab_run.result is not None
    ...
    _secondary = (
        mo.Html(
            render_captioned_table(
                ...
                caption=f"Content-type evidence for seed {lab_run.seed}",
