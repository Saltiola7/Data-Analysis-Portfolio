...
# ]
# ///
"""Interactive synthetic health-risk quality learning lab."""
...
def _():
    ...
    Audit duplicate fictional profiles before computing a descriptive ordinal association. Fixture identity displays `generator_version`, seed, row count, analysis grain, and canonical CSV SHA-256.
    ...
    **Limitations:** This fictional data has no clinical validity. Risk bands are assigned independently from the analyzed score composite. Results do not diagnose, predict individual outcomes, estimate treatment effects, imply causality, or recommend medical action.
    """)
    return
    ...
    _table = (
        mo.Html(
            render_captioned_table(
                ...
                caption=f"Unique-profile risk-band evidence for seed {lab_run.seed}",
...
def _(lab_run):
    _chart = (
        mo.Html(
            render_bar_evidence(
                lab_run.result.primary_table,
                label_column="risk_band",
                value_column="unique_profiles",
                title="Unique fictional profiles by risk band",
                description=(
                    "Horizontal bars count unique fictional profiles after "
    ...
    _secondary = (
        mo.Html(
            render_captioned_table(
                ...
                caption=f"Duplicate-profile audit for seed {lab_run.seed}",
                test_id="secondary-table",
            )
        )
        if lab_run.result is not None and lab_run.result.secondary_table is not None
        else mo.md("*No duplicate-profile rows are available.*")
    )
    _secondary
    return
...
def _(lab_run):
    ...
    mo.callout(f"Safety boundary: {_notes}", kind="info")
    return
